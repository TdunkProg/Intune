...
try{
  Set-ItemProperty -Path Registry::"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\IntuneManagementExtension\SideCarPolicies" -Name "ManagedInstallerEnabled" -Value 1 -Force

  Writelog ('Sidecar is set as the managed Installer.')
}
catch{

  $e = $_.Exception
  WriteLog ('Failed to enable managed installer in the registry ' + $e.Message.ToString())
  exit 1   
}
